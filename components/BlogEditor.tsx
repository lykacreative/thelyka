...
function resolveCoverField(
  ...
  // Legacy local media path support.
  ...
  // If the field already contains a Cloudinary URL,
  // return it directly.
  if (
  ...
  const found = localImages.find(
    (img) =>
      img.filename === trimmed &&
      (!year || img.year === year)
  );
  ...
  // Try again without requiring the year.
  const foundAnywhere = localImages.find(
    (img) => img.filename === trimmed
  );

  if (foundAnywhere) {
    return foundAnywhere.src;
  }

  // Not found - return the value as-is.
  return trimmed;
}
...
export function BlogEditor({
  ...
  const [existingPosts, setExistingPosts] =
    useState<ExistingPost[]>([]);
  ...
  const [paneOpen, setPaneOpen] =
    useState(false);

  const contentRef =
    useRef<HTMLTextAreaElement | null>(null);

  // Local copy allows newly uploaded images to appear immediately.
  const [localImages, setLocalImages] =
    useState<BlogImage[]>(allImages);
  ...
  async function handleDeleteImage(
    ...
    if (!img.publicId) {
      setStatus({
        kind: "error",
        message:
          "This image cannot be deleted because its Cloudinary ID is missing.",
      });
    ...
    const confirmed = window.confirm(
      `Delete "${img.filename}" permanently from Cloudinary?`
    );
    ...
    try {
      ...
      setStatus({
        kind: "success",
        message: `Image deleted: ${img.filename}`,
      });
  ...
  async function handleUpload(
    ...
    try {
      ...
      if (
        ...
        // Automatically use the uploaded image
        // as the cover if there isn't one.
        setForm((prev) => ({
          ...prev,
          year:
            payload.year ||
...
![image alt](PASTE-CLOUDINARY-IMAGE-URL-HERE)
:::
...
:::`;
    ...
    const newContent =
      `${before}${prefix}${template}${suffix}${after}`;
    ...
    requestAnimationFrame(() => {
      ...
      const selectEnd =
        selectStart +
        "PASTE-CLOUDINARY-IMAGE-URL-HERE".length;
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <label className="block">
              ...
              <input
                ...
                placeholder="Cloudinary image URL or filename"
                className="w-full border border-[var(--frame)] bg-[var(--page-bg-solid)] px-3 py-2 font-display text-base tracking-normal text-[var(--page-fg)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--frame)]"
              />
            </label>
          </div>
...
:::split
![alt](PASTE-CLOUDINARY-IMAGE-URL-HERE)
:::
          ...
          <div className="grid max-h-[300px] grid-cols-4 gap-2 overflow-y-auto sm:grid-cols-6 lg:grid-cols-8">
            {filteredImages.map(
              (img) => (
                <div
                  key={img.src}
                  className="group relative aspect-square overflow-hidden border border-[var(--frame)] bg-[var(--panel-bg)]"
                >
                  <button
                    type="button"
                    onClick={() =>
                    ...
                    className="absolute inset-0 h-full w-full transition hover:ring-2 hover:ring-[var(--frame)]"
                  ...
                  <button
                    ...
                    className="absolute right-1 top-1 z-10 grid h-6 w-6 place-items-center rounded-full bg-black/70 text-white opacity-0 transition hover:bg-rose-700 group-hover:opacity-100"
          ...
          >
            ...
            >
              <div>
                ...
                {uploadPreview && (
                  <div className="relative mt-3 aspect-square w-full max-w-[180px] overflow-hidden border border-[var(--frame)] bg-[var(--panel-bg)]">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img
                      src={
                        uploadPreview
